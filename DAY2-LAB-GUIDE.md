...
aws secretsmanager create-secret \
  ...
  --secret-string '{"username":"admin","password":"WorkshopDB2026!","engine":"mysql","host":"'$(aws rds describe-db-instances --db-instance-identifier workshop-db --query 'DBInstances[0].Endpoint.Address' --output text --region us-east-1)'","port":3306,"dbname":"cloudkida"}' \
  --region us-east-1
```
...
```json
{
  ...
  "Statement": [{
    ...
    "Resource": "arn:aws:secretsmanager:us-east-1:*:secret:workshop/*"
  }]
}
```
...
cat > /tmp/secrets-policy.json << EOF
{
  ...
  "Statement": [{
    ...
    "Resource": "arn:aws:secretsmanager:us-east-1:${ACCOUNT_ID}:secret:workshop/*"
  }]
}
EOF
...
2. In the container definition, change the DB_PASSWORD from **Value** to **ValueFrom**:
   ...
   - Add under **Secrets**: 
     - Key: `DB_PASSWORD`
     - Value: `arn:aws:secretsmanager:us-east-1:<account-id>:secret:workshop/db-credentials`
     - JSON key: `password`
3. Click **Create**

**CLI:**
```bash
SECRET_ARN=$(aws secretsmanager describe-secret --secret-id workshop/db-credentials --query 'ARN' --output text --region us-east-1)
...
aws ecs register-task-definition --cli-input-json file:///tmp/task-definition-v2.json --region us-east-1
```
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:logs:us-east-1:*:log-group:/ecs/workshop-app:*"
    }
  ]
}
EOF
...
aws ecs describe-task-definition --task-definition workshop-app --query 'taskDefinition.containerDefinitions[0].secrets' --region us-east-1
```
...
aws ec2 create-tags --resources $VPCE_SG --tags Key=Name,Value=workshop-vpce-sg --region us-east-1
aws ec2 authorize-security-group-ingress --group-id $VPCE_SG \
  --protocol tcp --port 443 --cidr 10.0.0.0/16 --region us-east-1
...
for SERVICE in com.amazonaws.us-east-1.ecr.dkr com.amazonaws.us-east-1.ecr.api com.amazonaws.us-east-1.secretsmanager com.amazonaws.us-east-1.logs; do
...
PRIV_RT=$(aws ec2 describe-route-tables --filters "Name=tag:Name,Values=workshop-private-rt" --query 'RouteTables[0].RouteTableId' --output text --region us-east-1)
...
DATA_NACL=$(aws ec2 create-network-acl --vpc-id $VPC_ID \
  ...
  --query 'NetworkAcl.NetworkAclId' --output text --region us-east-1)
...
aws ec2 create-network-acl-entry --network-acl-id $DATA_NACL --ingress \
  ...
  --cidr-block 10.0.10.0/24 --rule-action allow --region us-east-1

aws ec2 create-network-acl-entry --network-acl-id $DATA_NACL --ingress \
  --rule-number 110 --protocol tcp --port-range From=3306,To=3306 \
  --cidr-block 10.0.11.0/24 --rule-action allow --region us-east-1

aws ec2 create-network-acl-entry --network-acl-id $DATA_NACL --ingress \
  --rule-number 200 --protocol tcp --port-range From=1024,To=65535 \
  --cidr-block 10.0.0.0/16 --rule-action allow --region us-east-1
...
for SUBNET in $DATA_SUBNET_1 $DATA_SUBNET_2; do
  ASSOC_ID=$(aws ec2 describe-network-acls --filters "Name=association.subnet-id,Values=$SUBNET" --query 'NetworkAcls[0].Associations[?SubnetId==`'$SUBNET'`].NetworkAclAssociationId' --output text --region us-east-1)
  aws ec2 replace-network-acl-association --association-id $ASSOC_ID --network-acl-id $DATA_NACL --region us-east-1
done
```
...
```bash
# Create log group
aws logs create-log-group --log-group-name /vpc/workshop-flow-logs --region us-east-1
...
aws kms create-alias --alias-name alias/workshop-key \
  --target-key-id $KMS_KEY_ID --region us-east-1
...
**CLI:**
```bash
ALB_ARN=$(aws elbv2 describe-load-balancers --names workshop-alb --query 'LoadBalancers[0].LoadBalancerArn' --output text --region us-east-1)
...
TOPIC_ARN=$(aws sns create-topic --name workshop-alerts --query 'TopicArn' --output text --region us-east-1)

# Subscribe your email
aws sns subscribe --topic-arn $TOPIC_ARN \
  --protocol email --notification-endpoint your-email@example.com --region us-east-1
...
aws cloudwatch put-metric-alarm \
  ...
  --dimensions Name=LoadBalancer,Value=$(aws elbv2 describe-load-balancers --names workshop-alb --query 'LoadBalancers[0].LoadBalancerArn' --output text --region us-east-1 | cut -d'/' -f2-) \
  --region us-east-1
...
aws ec2 describe-vpc-endpoints --filters "Name=vpc-id,Values=$VPC_ID" \
  --query 'VpcEndpoints[*].{Service:ServiceName,State:State}' --output table --region us-east-1
...
aws cloudwatch describe-alarms --alarm-name-prefix workshop \
  --query 'MetricAlarms[*].{Name:AlarmName,State:StateValue}' --output table --region us-east-1
```
...
LOCK_TOKEN=$(aws wafv2 get-web-acl --name workshop-waf --scope REGIONAL --id $(echo $WAF_ARN | rev | cut -d'/' -f1 | rev) --query 'LockToken' --output text --region us-east-1)
aws wafv2 delete-web-acl --name workshop-waf --scope REGIONAL --id $(echo $WAF_ARN | rev | cut -d'/' -f1 | rev) --lock-token $LOCK_TOKEN --region us-east-1

# 2. Delete VPC Endpoints
for VPCE in $(aws ec2 describe-vpc-endpoints --filters "Name=vpc-id,Values=$VPC_ID" --query 'VpcEndpoints[*].VpcEndpointId' --output text --region us-east-1); do
...
aws cloudwatch delete-alarms --alarm-names workshop-alb-5xx workshop-ecs-cpu-high workshop-rds-connections --region us-east-1

# 6. Delete SNS Topic
aws sns delete-topic --topic-arn $TOPIC_ARN --region us-east-1

# 7. Delete Secrets Manager secret
aws secretsmanager delete-secret --secret-id workshop/db-credentials --force-delete-without-recovery --region us-east-1

# 8. Delete KMS key (scheduled)
aws kms schedule-key-deletion --key-id $KMS_KEY_ID --pending-window-in-days 7 --region us-east-1
